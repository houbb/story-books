...
### Web / 移动端统一自然分页与无滚动体验
- **自适应内容自然切页**：优化 `ParagraphPageBreakStrategy` 分页策略，调整基准安全字符容量（320 字/页）并引入智能句末断句（标点符号优先断开），彻底杜绝 Web 端与移动端多出内容被截断遮挡。
- **字号设置动态联动**：`BookPaginator` 支持根据用户阅读字号动态计算单页安全容量（大字号自动多分页，小字号优雅排版），全书物理总页数与章节 TOC 始终保持一致与精确。
- **去除纵向滚动条**：移动端与 Web 端统一为“一页一屏、左右翻页/滑动”，单页高度 100% 贴合，消除上下滚动条与滑动冲突，还原纯粹舒畅的纸质翻书阅读感。
